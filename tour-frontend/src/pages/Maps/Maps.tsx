...
const Maps: React.FC = () => {
  ...
  useEffect(() => {
    if (mapFocusLocation) {
      ...
      // 마커에 추가 (중복 체크)
      const isDuplicate = mapMarkers.some(marker => marker.placeId === mapFocusLocation.placeId);
      if (!isDuplicate) {
  ...
  const handleRouteSearch = useCallback(async () => {
    ...
    try {
      ...
      if (timeOption === 'now') {
        ...
        departureTime = new Date(new Date(selectedDateTime).getTime() - 60 * 60 * 1000);
      } else {
        departureTime = new Date();
      }
      ...
      const routes = await searchTransitRoutes({
        ...
        departureTime,
        arrivalTime: timeOption === 'arrive' && selectedDateTime ? new Date(selectedDateTime) : undefined
      });
  ...
  const handleAddRouteToSchedule = useCallback((route: RouteResult) => {
    ...
    try {
      ...
      const isDuplicate = savedRoutes.some((saved: any) => 
      ...
      if (!isDuplicate) {
        ...
        console.log('이미 저장된 경로입니다:', route.departure, '->', route.destination);
      }
    } catch (error) {
      ...
      <Box sx={{ p: 2 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', my: 1 }}>
          <IconButton
            ...
            title="출발지와 목적지 교체"
          >
            <SwapVertIcon color={departureLocation || destinationLocation ? 'primary' : 'disabled'} />
          </IconButton>
        </Box>
        ...
        <Box sx={{ mb: 2 }}>
          <FormControl fullWidth size="small">
            ...
            <Select
              value={timeOption}
              label="출발 시간 옵션"
              onChange={(e) => setTimeOption(e.target.value as 'now' | 'depart' | 'arrive')}
            >
              ...
              <Card key={index} sx={{ mb: 2 }}>
                <CardContent>
                  ...
                  <Typography variant="body2" gutterBottom>
                    {formatTimeTo24Hour(route.departureTime)} → {formatTimeTo24Hour(route.arrivalTime)}
                  </Typography>
                        ...
                        return (
                          <Box key={stepIndex} sx={{ display: 'flex', alignItems: 'center', mt: 0.5 }}>
                            ...
                            <Typography variant="caption">
                              {step.line} ({step.departure} {formatTimeTo24Hour(step.departureTime)} → {step.arrival} {formatTimeTo24Hour(step.arrivalTime)})
                            </Typography>
                          </Box>
                        );
                      })
